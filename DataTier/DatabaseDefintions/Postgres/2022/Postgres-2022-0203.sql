DROP database datasynthesis;
CREATE database datasynthesis;
USE datasynthesis;
...
CREATE TABLE platform_dataattributes (
                                         ...
                                         RegisteredApp char(38) DEFAULT 'NULL',
                                         PlatformTableName varchar(38) DEFAULT 'NULL',
...
(
    termcodesetid integer      default nextval('terms_codeset_hl7v2_termcodesetid_seq'::regclass) not null
        constraint terms_codeset_hl7v2_pkey primary key,
    codesetsid bigint not null
        constraint terms_codeset_hl7v2_codesetsid_fkey
            references refdata_codeset,
    createddate   timestamp    default CURRENT_TIMESTAMP,
    statusid      integer      default 1
        constraint terms_codeset_hl7v2_statusid_fkey
    ...
    industrystd   varchar(6)   default 'UNDF'::character varying
        constraint terms_codeset_industrystd_industrystd_fkey
...
create index terms_codeset_industrystd_index
    on terms_codeset_industrystd (termcodesetid, codesetsid, createddate, statusid, codevalue, codedesc, industrystd);

create unique index terms_codeset_industrystd_uindex
    on terms_codeset_industrystd (codesetsid, codevalue, codedesc, industrystd);

create index terms_umls_mrconoso_index
    on terms_umls_mrconoso (cui, lat, ts, lui, stt, sui, ispref, aui, saui, scui, sdui, sab, tty, code, str, srl,
                            suppress, cvf);


create unique index dataexisting_namefirst_uindex
    on dataexisting_namefirst (firstname, gender, registeredapp);

create index dataexisting_namefirst_idx
    on dataexisting_namefirst (firstnameid, firstname, gender, statusid, createddate, createduser, registeredapp);

create unique index dataexisting_namelast_uindex
    on dataexisting_namelast (lastname, registeredapp);

create index dataexisting_namelast_index
    on dataexisting_namelast (lastnameid, lastname, statusid, createddate, createduser, registeredapp);

create index datagenerated_accountnumbers_index
    on datagenerated_accountnumbers (accountnumbersid, accountnumbervalue, createddate, createduser, statusid,
                                     registeredapp, datagentypeid);

create unique index datagenerated_accountnumbers_uindex
    on datagenerated_accountnumbers (accountnumbervalue, registeredapp, datagentypeid);

create index datagenerated_addresses_index
    on datagenerated_addresses (addressid, addressstreet, addressstreet2, createddate, createduser, statusid,
                                registeredapp, datagentypeid);

create unique index datagenerated_addresses_uindex
    on datagenerated_addresses (addressstreet, addressstreet2, registeredapp, datagentypeid);

create index datagenerated_bankaccount_index
    on datagenerated_bankaccount (bankaccountsid, bankaccountvalue, createddate, statusid, createduser, registeredapp,
                                  datagentypeid);

create unique index datagenerated_bankaccount_uindex
    on datagenerated_bankaccount (bankaccountvalue, registeredapp, datagentypeid);

create index datagenerated_creditcard_index
    on datagenerated_creditcard (creditcardid, creditcardnumber, creditcardname, createddate, statusid, createduser,
                                 registeredapp, datagentypeid);

create unique index datagenerated_creditcard_uindex
    on datagenerated_creditcard (creditcardnumber, creditcardname, registeredapp, datagentypeid);

create index datagenerated_dateofbirth_index
    on datagenerated_dateofbirth (dateofbirthsid, dateofbirth, age, dateofbirthdate, createddate, statusid, createduser,
                                  registeredapp);

create unique index datagenerated_dateofbirth_uindex
    on datagenerated_dateofbirth (dateofbirthdate, dateofbirth, dateofbirthdate, registeredapp);

create index datagenerated_driverslicenses_index
    on datagenerated_driverslicenses (driverslicensesid, dln, statecode, createddate, statusid, datagentypeid,
                                      createduser, registeredapp);

create unique index datagenerated_driverslicenses_uindex
    on datagenerated_driverslicenses (dln, statecode, datagentypeid, registeredapp);

create index datagenerated_ein_index
    on datagenerated_ein (einid, einvalue, createddate, statusid, createduser, registeredapp);

create unique index datagenerated_ein_uindex
    on datagenerated_ein (einvalue, registeredapp);

create index datagenerated_phonenumber_index
    on datagenerated_phonenumber (phonenumberid, phonenumbervalue, createddate, statusid, createduser, registeredapp);

create unique index datagenerated_phonenumber_uindex
    on datagenerated_phonenumber (phonenumbervalue, registeredapp);

create index datagenerated_phonenumbersintl_index
    on datagenerated_phonenumbersintl (phonenumberintlid, phonenumbervalue, createddate, statusid, countryid,
                                       createduser, registeredapp);

create unique index datagenerated_phonenumbersintl_uindex
    on datagenerated_phonenumbersintl (phonenumbervalue, countryid, registeredapp);

create index datagenerated_socialsecuritynumber_index
    on datagenerated_socialsecuritynumber (socialsecuritynumberid, socialsecuritynumbervalue, createddate, statusid,
                                           createduser, registeredapp);

create unique index datagenerated_socialsecuritynumber_uindex
    on datagenerated_socialsecuritynumber (socialsecuritynumbervalue, registeredapp);

create index datagenerated_useridentities_index
    on datagenerated_useridentities (useridentitiesid, useridentityvalue, userdomain, additionalattributes, createddate,
                                     statusid, registeredapp, datagentypeid);

create unique index datagenerated_useridentities_uindex
    on datagenerated_useridentities (useridentityvalue, userdomain, additionalattributes, registeredapp);
