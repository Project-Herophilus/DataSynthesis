...
create definer = admin@`%` trigger refdata_organization_uuid
	before insert
...
create definer = admin@`%` trigger refdata_application_uuid
	before insert
...
(
	...
	constraint FK_refdata_codesetvalues_apps
		foreign key (ApplicationID) references refdata_application (ApplicationID),
...
create index IDX_refdata_codesetvalues
	on refdata_codesetvalues (CodeSetValueID, CodesetID, QtyValue, QtyValueDesc, ApplicationID, StatusID, CreatedDate);

create index IX_RefData_Vendors
	on refdata_vendor (VendorID, VendorName, StatusID, VendorGUID, CreatedDate, CreatedUser);

create definer = admin@`%` trigger refdata_vendor_uuid
	before insert
	on refdata_vendor
	for each row
	BEGIN
