...
CREATE TABLE IF NOT EXISTS `dataexisting_zipcodeus` (
    ...
    `Lattitude` VARCHAR(10) NULL DEFAULT NULL,
    `Longitude` VARCHAR(10) NULL DEFAULT NULL,
    `Lctn` VARCHAR(99) NULL DEFAULT NULL,
    `CreatedDate` DATETIME NULL DEFAULT CURRENT_TIMESTAMP,
    `StatusID` SMALLINT NULL DEFAULT '1',
    `CreatedUser` VARCHAR(20) NULL DEFAULT NULL,
    `RegisteredApp` CHAR(38) NULL DEFAULT NULL,
...
(
    ImplCodeSetID bigint auto_increment
        primary key,
    ...
    constraint FK_implcodesets_Application
        foreign key (ApplicationID) references impl_application (AppGUID),
    constraint FK_implcodesets_Org
        foreign key (OrganizationID) references impl_organization (OrganizationGUID),
    constraint FK_implcodesets_codeset
        foreign key (CodeSetsID) references refdata_codeset (CodeSetsID),
    constraint FK_implcodesets_industrystd
        foreign key (IndustryStd) references refdata_industrystd (IndustryStd),
    constraint FK_implcodesets_sensitivityflag
        foreign key (SensitivityFlagID) references refdata_sensitivityflag (SensitiveFlagID),
    constraint FK_implcodesets_status
        foreign key (StatusID) references refdata_status (StatusID)
)
    ENGINE = InnoDB
    DEFAULT CHARACTER SET = latin1;

create index IDX_ImplCodeSets
    on impl_codesets (ImplCodeSetID, CodeSetsID, CreatedDate, OrganizationID,
                      ApplicationID, Industrystd,FieldMapping, SensitivityFlagID, ExternalTableID,
                      ExternalNotes, ExternalLink, StatusID);
